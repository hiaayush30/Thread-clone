  ...
  return (
    <div className='w-full h-full flex justify-around items-center text-black'>
      <NavLink to={'/'} className={({ isActive }) => (
        isActive ? "p-2 dark:text-zinc-100 rounded-full dark:shadow-md dark:md:shadow-white" : "p-2 text-slate-400 dark:text-zinc-500"
      )}>
        <IoHomeSharp title="home" size={32} className='cursor-pointer hover:scale-110 transition-all duration-300 ease-in-out' />
      </NavLink>
      <NavLink to={'/search'} className={({ isActive }) => (
        isActive ? "p-2 dark:text-zinc-100 dark:shadow-md dark:md:shadow-white rounded-full" : "p-2 text-slate-400 dark:text-zinc-500"
      )}>
      ...
      <NavLink to={`/profile/threads/${myInfo._id}`} className={({ isActive }) => (
        isActive ? "p-2 rounded-full dark:text-zinc-100 dark:shadow-md dark:md:shadow-white" : "p-2 text-slate-400 dark:text-zinc-500"
      )}>
